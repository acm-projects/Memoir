...
export default function BulletinBoard() {
  ...
  const [items, setItems] = useState<Item[]>([
    { id: '2', type: 'card', content: 'card1', x: 30, y: 260, image: require('../../assets/images/cards.jpg'), rotation: seededRotation('2') },
    { id: '3', type: 'card', content: 'card2', x: 200, y: 400, image: require('../../assets/images/card2.jpg'), rotation: seededRotation('3') },
    { id: '4', type: 'card', content: 'card3', x: 60, y: 550, image: require('../../assets/images/card3.jpg'), rotation: seededRotation('4') },
  ]);
  ...
  const addNote = (color: string) => {
    ...
    setItems(prev => [...prev, { id, type: "note", content: "New note", x: 80, y: 100, color, rotation: seededRotation(id) }]);
    setActiveTool(null);
  };

  const addSticker = (stickerKey: string) => {
    const id = Date.now().toString();
    setItems(prev => [...prev, { id, type: "sticker", content: "", x: 150, y: 200, sticker: stickerKey, rotation: seededRotation(id) }]);
    setActiveTool(null);
  };
  ...
  async function pickImage() {
    ...
    if (!res.canceled && res.assets[0]?.uri) {
      const id = Date.now().toString();
      setItems(prev => [...prev, { id, type: "sticker", content: "", x: 100, y: 150, sticker: res.assets[0].uri, rotation: seededRotation(id) }]);
      setActiveTool(null);
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {/* Right: Edit button, hidden while editing */}
        {!isEditing && (
          <TouchableOpacity
      ...
      <ImageBackground source={require('../../assets/images/layered-vintage-paper.png')} style={styles.paperBackground}>
        ...
        {isEditing && (
          <View style={styles.footerWrapper}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8 }}>
                      {gifs.map(item => (
                        <TouchableOpacity key={item.id} onPress={() => {
                          const id = Date.now().toString();
                          setItems(prev => [...prev, { id, type: "sticker", content: "", x: 100, y: 150, sticker: item.images.fixed_height.url, rotation: seededRotation(id) }]);
                          setActiveTool(null);
                        }}>
                          <Image source={{ uri: item.images.fixed_height.url }} style={styles.gifThumb} />
                        </TouchableOpacity>
                      ))}
                    </ScrollView>
                  </View>
            ...
            {/* Toolbar — note, photo, sticker, gif + checkmark on the right */}
            <View style={styles.toolbar}>
              <Pressable style={[styles.toolButton, activeTool === "note" && styles.activeToolBtn]} onPress={() => setActiveTool("note")}>
                <Ionicons name="document-text-outline" size={24} color="#5A390E" />
              </Pressable>
              <Pressable style={[styles.toolButton, activeTool === "photo" && styles.activeToolBtn]} onPress={() => setActiveTool("photo")}>
                <Ionicons name="image-outline" size={24} color="#5A390E" />
              </Pressable>
              <Pressable style={[styles.toolButton, activeTool === "sticker" && styles.activeToolBtn]} onPress={() => setActiveTool("sticker")}>
                <Ionicons name="happy-outline" size={24} color="#5A390E" />
              </Pressable>
              <Pressable style={[styles.toolButton, activeTool === "gif" && styles.activeToolBtn]} onPress={() => { setActiveTool("gif"); searchGifs(""); }}>
...
const styles = StyleSheet.create({
  ...
  topBanner: {
    height: 150, width: '105%', flexDirection: 'row', alignItems: 'flex-end',
  ...
  bannerTitle: {
    ...
    color: '#F6E5CD', fontFamily: 'Calistoga',
    marginBottom: 8,
  },
  ...
  corkboardFrame: {
    flex: 1, borderWidth: 12, borderColor: '#8B6A3E',
    borderRadius: 24, margin: 10, overflow: 'hidden',
    backgroundColor: 'rgba(139,106,62,0.04)', marginTop: 16,
  },
  board: { flex: 1 },
  ...
  toolbar: {
    ...
    backgroundColor: "#ede0cc", height: 60, width: "90%", borderRadius: 30,
    borderWidth: 1, borderColor: "#6D1B12", elevation: 5, shadowOpacity: 0.1, shadowRadius: 10,
    paddingHorizontal: 8,
  },
  ...
  panel: {
    backgroundColor: "#ede0cc", width: "92%", borderRadius: 20,
    padding: 16, borderWidth: 1, borderColor: "#d7c3ac",
  },
  ...
  panelUploadArea: {
    borderWidth: 1.5, borderColor: '#C8B89A', borderStyle: 'dashed',
    borderRadius: 12, padding: 25, alignItems: 'center', gap: 8,
  },
