...
// Type definitions (for reference only - JS file)
// TemplateMatch: {id, name, card_color, text_1-5, sticker_1-5, gif_1-2, similarity, custom_card_id}
// Item: {id, type, content, x, y, sticker, image, color, font, rotation, scale}
// Message: {id, role, text, templatePreview}// ─── Constants ────────────────────────────────────────────────────────────────
...
      const data = await response.json();
      ...
      const intent = data.design_intent ?? {};      if (suggestions.length === 0) {
...
        <Text style={{ color: '#f5ede0', fontSize: 16, fontWeight: '600' }}>
          {name || 'Chat'}
        </Text>
      </View>
...
        cardItems.stickers?.forEach((raw, i) => {
          try {
            const parsed = JSON.parse(raw);
